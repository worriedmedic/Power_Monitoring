...
with serial.Serial(addr,9600) as pt:
    ...
    while (1):
        ...
        if emoncms_update == 'true':
            url = 'https://emoncms.org/input/post.json?node=1&json={Temperature:%s,Pressure:%s,Humidity:%s,Voltage:%s,RSSI:%s}&apikey=4e6eff5d047580696f0e2a7ae9323983', str.(temp), str.(press), str.(humid), str.(volt), str.(rssi)
            r = requests.post(url)
            if verbose == 'true':
                print(r.text)
