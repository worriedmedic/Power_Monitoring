	...
	curr_time = time.strftime('%H:%M')
    	...
    	try:
			...
			try:
				...
				output = output.replace('CURDATE',str(today.strftime("%m/%d/%Y")))
				output = output.replace('CURTIME', str(curr_time))
				output = output.replace('SNRISE',str(sun_rise))
				output = output.replace('SNSET',str(sun_down))
				if internet:
				...
				output = output.replace('TDNTM',str(old.strftime('%H:%M')))
				output = output.replace('TDNLV',str(tide_pre_mag))
				output = output.replace('TDFTY',str(tide_next_type))
				output = output.replace('TDFTM',str(tide_datetime.strftime('%H:%M')))
